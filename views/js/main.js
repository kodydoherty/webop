...
var resizePizzas = function(size) {
  ...
  // Moved widnowwidth outside determinDX because it did not need to be recalculated everytime the function is called.
  ...
  // Queryselectorall is slower and it was being called multple times
  // stored getElmenetsByClassName once and used the array in the for loop

  // Pulled this logic outise of the for function because it returns the same values for every pizza.
  // Now the logic is only run once and the new width is added to every pizza.
  var pizzasArray = document.getElementsByClassName("randomPizzaContainer");
  var dx = determineDx(pizzasArray[0], size);
  var newwidth = (pizzasArray[0].offsetWidth + dx) + 'px';

  function changePizzaSizes(size) {
    for (var i = 0; i < pizzasArray.length; i++) {
      pizzasArray[i].style.width = newwidth;
    }
  }
...
function updatePositions() {
  ...
  // moved the scroll speed outside of the loop because it does not change as often
  var scrollSpeed = document.body.scrollTop / 1250;
  // there is repeating 5 options that were being printed. I hard coded the 5 options in an array
  // Then just interate over the array over and over again to simulate the pizza moves. This
  // drastically reduces the amount of calculations that need to be run.
  var speeds = [Math.sin(scrollSpeed + 0), Math.sin(scrollSpeed + 1), Math.sin(scrollSpeed + 2), Math.sin(scrollSpeed + 3), Math.sin(scrollSpeed + 4)];

  for (var i = 0; i < items.length; i++) {
    // Use the index % 5 to loop over the array over and over again.
    var phase = speeds[i % 5]
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // dropped 200 animations down to 40 because there are only 40 shown on the screen at any one time
  for (var i = 0; i < 40; i++) {
